...
class PreprocessingConductor:
    def __init__(self, scenarios: list[PreprocessScenario]):
        self.scenarios = scenarios
        self.best_preprocessor = None

    def get_best_preprocessor(self, np_image: np.array, ocr_engine: PaddleOCR):
        self._set_best_parameters_for_each_scenario(np_image, ocr_engine)
        self._set_best_preprocessor(np_image, ocr_engine)
        return self.best_preprocessor

    @staticmethod
    def _vote_for_best_image_index(np_images: list[np.ndarray]):
        # plot all images by batches
        # sort them with help of user input
        selector = ImageSelector(np_images)
        best_image_index = selector.best_image_index
        return best_image_index

    def _set_best_parameters_for_each_scenario(self, np_image: np.array, ocr_engine: PaddleOCR):
        for scenario in self.scenarios:
            ...
            # Define best image
            best_image_index = self._vote_for_best_image_index(images)
            scenario.best_params = parameters[best_image_index]

    def _set_best_preprocessor(self, np_image: np.array, ocr_engine: PaddleOCR):
        processed_images = []
        for scenario in self.scenarios:
            processed_image = scenario.best_preprocessor(np_image)
        ...
        # Define best image
        best_image_index = self._vote_for_best_image_index(processed_images)
        self.best_preprocessor = self.scenarios[best_image_index].best_preprocessor
...
class Preprocessor:
    ...
    @staticmethod
    def _run_preprocessing_pipeline(np_image: np.array, rendered_code: str) -> np.array:
